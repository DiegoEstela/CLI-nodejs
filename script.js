...
export async function renameFiles(microFrontName){
    await fs.rename( `./packages/${microFrontName}/src/uma-template.tsx`, `./packages/${microFrontName}/src/uma-${microFrontName}.tsx` ,(err) => {
        if (err) throw err;
...
export async function createMicroFront (microFrontName){
        copydir('./template', `./packages/${microFrontName}`, {
            utimes: true, 
            mode: true,   
            cover: true    
          }, function(err){
